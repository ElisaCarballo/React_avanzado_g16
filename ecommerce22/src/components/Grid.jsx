...
const Grid = ({ search }) => {
  ...
  useEffect(() => {
    ...
    if (option == 'news') {
      ...
      const sort_date = dates.sort((a, b) => {
    ...
    <>
      <div className='body-grid'>
        ...
        <div className='grid-container'>
          {
            ...
            !search
              ? items.slice(page, page + itemsxPagina)
                .map(item => (
        ...
        <div className='options-container'>
          <button className={`all ${option == 'all' ? 'selected' : ''}`} onClick={() => handleOption('all')} />
          <br />
          <button className={`news ${option == 'news' ? 'selected' : ''}`} onClick={() => handleOption('news')} />
        </div>
